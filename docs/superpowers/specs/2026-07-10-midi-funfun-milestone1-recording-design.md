...
- マイク入力キャプチャ(モノラル前提)
- 入力レベルメーター(ピークホールド表示)
- 録音中のモニタリング(入力を出力へパススルー、既定OFF・トグルでON可 — 2026-07-12追記: 下記参照)
...
- `processBlock`: 入力バッファ(先頭チャンネル、モノラル前提)を`PeakLevelTracker`に渡す → `RecordingTransport::advance()`を呼ぶ → 結果に応じて現在テイクへサンプルを追記し、クリック音を出力にミックスし、モニタリングが有効な場合のみ入力を出力へパススルーする(既定は無効、無効時は出力を無音化する。理由は下記2026-07-12追記を参照)。
- Editor(UI)にツールバー要素を追加する:
  - BPM入力(数値editable)
  - メトロノームON/OFFトグル
  - モニタリングON/OFFトグル(既定OFF。2026-07-12追記を参照)
...
### 2026-07-12追記: モニタリングの既定OFF化とStandalone入力ミュート自動解除

Milestone 1完了後の手動確認で、JUCEのStandaloneラッパーが持つ入力ミュート安全機能(`StandalonePluginHolder`は入出力両方を持つプロセッサを検出すると`shouldMuteInput`を既定`true`にし、`processBlock`到達前に入力バッファをゼロクリアする)により、録音自体が機能しない不具合が発覚した。原因調査の結果、この安全機能は「モニタリングが常時ONで入力を出力へパススルーしている」という本設計の実装に対して正当な理由(スピーカー環境での実際のフィードバックループ)で発動していたと判明した。

対応として、モニタリング(パススルー)を既定OFFのユーザートグル方式に変更し、無効時は出力を全チャンネル無音化する(チャンネル0のバッファエイリアシングによる暗黙のパススルーも防ぐ)。モニタリングが既定OFFであればフィードバックの実害が無いため、Standalone起動時に`StandalonePluginHolder::getInstance()->getMuteInputValue().setValue(false)`を呼び自動的に入力ミュートを解除する(以後、ユーザーがモニタリングをONにした場合のフィードバックリスクは許容する — オプトインであるため)。
